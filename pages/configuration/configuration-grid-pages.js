...
class ConfigurationGridPage extends BasePage {
  selectors;
  elements;
  ...
  async configureGridbyCustomCosts(customCosts) {
    try {
      const gridHeading = this.elements.heading(uiConstants.headings.gridConnectionOptions);
      ...
      await valueInput.fill(customCosts || '65000');
      ...
      await this.verifyElementState(saveButton, true);
      await this.click(saveButton);
      await this.verifyElementVisibility(true, this.elements.alert(uiConstants.alerts.gridConnectionUpdated));
